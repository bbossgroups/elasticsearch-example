...
//				System.out.println(r);
     ...
     * @throws ParseException
     */
    public void testAddAndUpdateMapNoPersistentIdDocument() throws ParseException {
        //创建创建/修改/获取/删除文档的客户端对象，单实例多线程安全
        ClientInterface clientUtil = ElasticSearchHelper.getRestClientUtil();
        //构建一个对象，日期类型，字符串类型属性演示
        Map demo = new LinkedHashMap();
     ...
     * @throws ParseException
     */
    public void testAddAndUpdateMapNoPersistentIdDocuments() throws ParseException {
        //创建创建/修改/获取/删除文档的客户端对象，单实例多线程安全
        ClientInterface clientUtil = ElasticSearchHelper.getRestClientUtil();
        List<Map> datas = new ArrayList<Map>();
        //构建一个对象，日期类型，字符串类型属性演示
        ...
        addOptions.setPersistMapDocId(false);
        //如果orderId对应的文档已经存在则更新，不存在则插入新增
        String response = clientUtil.addDocuments("demonoid",//索引表
                datas,addOptions);
